...
class WeibosController < ApplicationController
  ...
  def sina_login
    ...
    redirect_to "#{request_token.authorize_url}&oauth_callback=http://#{request.env["HTTP_HOST"]}/weibos/sina_index"
  end
  ...
  def friend_add_request
    ...
    request_token = oauth.consumer.get_request_token
    session[:rtoken], session[:rsecret] = request_token.token, request_token.secret
    redirect_to "#{request_token.authorize_url}&oauth_callback=http://#{request.env["HTTP_HOST"]}/weibos/friend_add"
  end
